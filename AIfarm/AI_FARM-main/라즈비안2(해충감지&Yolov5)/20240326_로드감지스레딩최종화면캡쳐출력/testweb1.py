...
CAMERA_SPEED = 5  # Increase or decrease as needed
...
def detect_objects_thread():
    ...
    while detection_active:
        ...
        if frame_objects:
            ...
            last_detected_obj = frame_objects[-1]
            message = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {last_detected_obj['class_name']} 확률: {last_detected_obj['confidence']}%"
