...
def detect_objects_thread(vs):
    ...
    while detection_active:
        ...
        for detection in results.xyxy[0]:
            ...
            # Draw green box around the object
            cv2.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 2)
            
            # Display probability above the green box
            cv2.putText(img, f'{percent}%', (int(x1), int(y1) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
            ...
            message = f"{timestamp} 확률 : {percent}%"
...
if __name__ == '__main__':
    load_model()
    threading.Thread(target=detect_objects_thread, args=(VideoStream(src=0),)).start()
