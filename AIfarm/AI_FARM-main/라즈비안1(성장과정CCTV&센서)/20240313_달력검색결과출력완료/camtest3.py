...
def search_images():
    try:
        ...
        # 선택한 날짜에 해당하는 이미지들 중 마지막 이미지만 추출
        last_images = {}
        for image in images:
            date = image['name'][:8]
            if date in last_images:
                last_images[date] = max(last_images[date], image['path'])
            else:
                last_images[date] = image['path']

        last_images_list = [{'name': date, 'path': path} for date, path in last_images.items()]

        return jsonify({'images': last_images_list})
    except Exception as e:
        error_message = str(e)
